...
var source = (() => {
  ...
  var AUTH_TOKEN = "Bearer eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJWxINuIEhvw6BuZyDEkGluaCIsImNvbWljSWRzIjpbXSwicm9sZUlkIjpudWxsLCJncm91cElkIjpudWxsLCJhZG1pbiI6ZmFsc2UsInJhbmsiOjAsInBlcm1pc3Npb24iOltdLCJpZCI6IjAwMDA1MjYzNzAiLCJ0ZWFtIjpmYWxzZSwiaWF0IjoxNzE1NDI0NDU3LCJlbWFpbCI6Im51bGwifQ.EjYw-HvoWM6RhbNzJkp06sSh61leaPcND0gb94PlDKeTYxfxU-f6WaxINAVjVYOP0pcVcG3YmfBVb4FVEBqPxQ";
  var GOCTRUYENTRANH_CONSTANTS = {
  ...
  var HOME_SECTIONS = {
    ...
    UPDATED: {
      id: "new_updated",
      title: "TRUY\u1EC6N C\u1EACP NH\u1EACT G\u1EA6N \u0110\xC2Y"
    }
  };
  var ERROR_MESSAGES = {
  ...
  var GocTruyenTranhInterceptor = class extends import_types2.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers ?? {},
        ...{
  ...
  // src/GocTruyenTranh/GocTruyenTranhParser.ts
  init_buffer();
  var import_types4 = __toESM(require_lib(), 1);

  // src/GocTruyenTranh/GocTruyenTranhUtils.ts
  init_buffer();
  var import_types3 = __toESM(require_lib(), 1);

  // src/GocTruyenTranh/interfaces/GocTruyenTranhInterfaces.ts
  init_buffer();

  // src/GocTruyenTranh/GocTruyenTranhUtils.ts
  var GocTruyenTranhUtils = class {
    /**
     * Convert time string to Date object
     */
    static convertTime(timeAgo) {
      let time;
      let trimmed = Number((/\d*/.exec(timeAgo) ?? [])[0]);
      trimmed = trimmed == 0 && timeAgo.includes("a") ? 1 : trimmed;
      if (timeAgo.includes("gi\xE2y") || timeAgo.includes("secs")) {
        time = new Date(Date.now() - trimmed * 1e3);
        ...
        time = new Date(Date.now() - trimmed * 864e5 * 7);
      } else if (timeAgo.includes("th\xE1ng")) {
        time = new Date(Date.now() - trimmed * 864e5 * 7 * 4);
        ...
        if (timeAgo.includes(":")) {
          ...
          const finalD = `${month}/${day}/${(/* @__PURE__ */ new Date()).getFullYear()}`;
          time = /* @__PURE__ */ new Date(`${finalD} ${H}`);
        } else {
          const [day, month, year] = timeAgo.split("-");
          time = /* @__PURE__ */ new Date(`${month}/${day}/${year}`);
        }
      }
      return time;
    }
     ...
     */
    static validateResponse(response, rules) {
      const errors = [];
      for (const rule of rules) {
        ...
        if (value !== void 0 && typeof value !== rule.type) {
          errors.push(
     ...
     */
    static handleCloudFlareError(status, method) {
      if (status === 503 || status === 403) {
        throw new import_types3.CloudflareError({ url: GOCTRUYENTRANH_DOMAIN, method });
      }
    }
    /**
     * Clean and encode manga ID
     */
    static cleanMangaId(mangaId) {
  ...
  // src/GocTruyenTranh/GocTruyenTranhParser.ts
  var GocTruyenTranhParser = class {
    parseMangaDetails($2, mangaId) {
      const titles = [
        GocTruyenTranhUtils.decodeHTMLEntity($2(".v-card-title").text().trim())
      ];
      let author = "", artist = "", status = "";
      $2(".information-section > div").each((_, obj) => {
        const label = $2(obj).text().trim().split("\n")[0];
        const value = $2(obj).text().split("\n")[1]?.trim() ?? "";
        switch (label) {
          case "T\xE1c gi\u1EA3:":
            author = artist = value;
            break;
      ...
      const image = this.processImageUrl($2(".v-image > img").attr("src"));
      const desc = GocTruyenTranhUtils.decodeHTMLEntity(
        $2(
          ".v-card-text.pt-1.px-4.pb-4.text-secondary.font-weight-medium"
        ).text()
      );
      const rating = parseFloat($2(".pr-3 > b").text().trim());
      const tags = [];
      $2(".group-content a").each((_, obj) => {
        const label = $2("span:nth-child(2)", obj).text().trim();
        const id = $2(obj).attr("href")?.trim().split("=")[1] ?? label;
    ...
    parseChapterList(data2, sourceManga) {
      const chapters = [];
      for (const obj of data2.result.chapters) {
        const chapNum = parseFloat(obj.numberChapter);
        const id = `chuong-${chapNum}`;
        const time = GocTruyenTranhUtils.convertTime(obj.stringUpdateTime);
    ...
    parseSearch(data2) {
      const items = [];
      const collectedIds = [];
      const mangaItems = data2.result.data ?? [];
      for (const obj of mangaItems) {
        ...
        if (!collectedIds.includes(mangaId)) {
          items.push({
            ...
            title: GocTruyenTranhUtils.decodeHTMLEntity(obj.name),
            subtitle: obj.chapterLatest[0] ? `Chapter ${obj.chapterLatest[0]}` : ""
          });
    ...
    parseTags(data2) {
      const tags = data2.result.map((cat) => ({
        id: `category:${cat.id}`,
        title: cat.name
      }));
      return [
    ...
    processImageUrl(url) {
      if (!url) return "";
      return url.indexOf("https") === -1 ? `${GOCTRUYENTRANH_DOMAIN}${url}` : url;
    }
  };
  ...
  var GocTruyenTranhExtension = class {
    ...
    async getDiscoverSectionItems(section, metadata) {
      ...
      switch (section.id) {
        case HOME_SECTIONS.HOT.id:
          url = GocTruyenTranhUtils.buildUrl(
            `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.SEARCH}/view`,
            { p: page.toString() }
          );
        ...
        case HOME_SECTIONS.NEW.id:
          url = GocTruyenTranhUtils.buildUrl(
            `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.SEARCH}/new`,
            { p: page.toString() }
          );
        ...
        case HOME_SECTIONS.UPDATED.id:
          url = GocTruyenTranhUtils.buildUrl(
            `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.SEARCH}/recent`,
            { p: page.toString() }
          );
    ...
    async getMangaDetails(mangaId) {
      ...
      const url = `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}/truyen/${cleanId.split("::")[0]}`;
      const $2 = await this.getHTML(url);
      return this.parser.parseMangaDetails($2, mangaId);
    }
    async getChapters(sourceManga) {
      const [, id] = sourceManga.mangaId.split("::");
      const url = `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.COMIC}/${id}/chapter?offset=0&limit=-1`;
      const data2 = await this.getJSON(url);
      return this.parser.parseChapterList(data2, sourceManga);
    }
    async getChapterDetails(chapter) {
      const [, mangaNum] = chapter.sourceManga.mangaId.split("::");
      const chapNum = chapter.chapterId.split("-")[1];
      const response = await this.getJSON(
        `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.CHAPTER}`,
        {
          method: "POST",
          headers: {
    ...
    async getSearchResults(query, metadata) {
      ...
      const searchUrl = query.title ? GocTruyenTranhUtils.buildUrl(`${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.SEARCH}`, {
        name: query.title
      }) : GocTruyenTranhUtils.buildUrl(
        `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.SEARCH}/category`,
        { p: page.toString(), value: genreIncluded[0] }
      );
      const response = await this.getJSON(searchUrl);
      const results = this.parser.parseSearch(response);
      return {
        items: results,
        metadata: query.title ? void 0 : { page: page + 1 }
      };
    }
    async getSearchTags() {
      const url = `${GOCTRUYENTRANH_CONSTANTS.DOMAIN}${API_ENDPOINTS.CATEGORY}`;
      const data2 = await this.getJSON(url);
      return this.parser.parseTags(data2);
    }
